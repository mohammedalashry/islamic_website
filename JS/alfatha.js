...
fetch(`https://api.alquran.cloud/v1/surah/${surahNumber}`).then(function (result)
    {
                 ...
                 let bsmallah="";
                 let surahContent="";
                 bsmallah=bsmallahmain.slice(0,40);
                 surahContent=result.data.ayahs[1].text + " (1) ";

                 

                 for (let j=2;j<result.data.ayahs.length;j++){
                    surahContent= surahContent+result.data.ayahs[j].text+" ("+j+") ";
                 }
